...
function richtextShow()
{
	...
	if(isset($_POST['act']))
	{
		switch($_POST['act'])
		{
			case 'save':
			default:
				$query = "UPDATE `{$RSadapter->tbl_rsform_forms}` SET `$openerId` = '".addslashes($_POST[$openerId])."' WHERE FormId = '$formId'";
...
function componentsValidateName($option)
{
	...
	if(eregi('[^a-zA-Z0-9_ ]', $componentName ))
	{
...
function submissionsDelete($option, $all=1)
{
	...
	else
	{
		...
		while($SubmissionId = mysql_fetch_array($query)){
			...
			deleteSubmissionFiles($SubmissionId['SubmissionId'], $formId);
...
function backupRestore($option)
{
	...
	rsform_HTML::backupRestore( _RSFORM_BACKEND_BACKUPRESTORE_TITLE_HEAD, $option, 'component', '', dirname(__FILE__), "");
}
...
function mappingsGetColumns($option)
{
	...
	foreach($columns as $column) $result.='<option value="'.$column.'">'.$column.'</option>';
	$result.='</select>';
	echo $result;
}
...
{
	$RSadapter = $GLOBALS['RSadapter'];
	$q="select * from `".$RSadapter->tbl_rsform_mappings."` where ComponentId='$_GET[ComponentId]' and MappingTable='$_GET[MappingTable]' and MappingColumn='$_GET[MappingColumn]'";
	mysql_query($q) or die($q.'<br>'.mysql_error());
	...
	$q="insert into `".$RSadapter->tbl_rsform_mappings."` (ComponentId, MappingTable, MappingColumn) values('$_GET[ComponentId]','$_GET[MappingTable]','$_GET[MappingColumn]')";
	mysql_query($q) or die(mysql_error());
...
{
	$RSadapter = $GLOBALS['RSadapter'];
	$q="delete from `".$RSadapter->tbl_rsform_mappings."` where MappingId='$_GET[MappingId]'";
	mysql_query($q) or die($q.'<br>'.mysql_error());
	$result=RSwebserviceMappingsTable($_GET['FormId']);
	echo $result;
}
...
function pluginsRemove($option)
{
	...
	if($plugin)
	{
		if($RSadapter->config['absolute_path'].'/components/com_rsform/plugins/'.$plugin.'.php')
		{
